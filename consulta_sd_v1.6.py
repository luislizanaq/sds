...
st.title("📊 Generador de Excel Formateado")

st.write("Sube una planilla Excel y obtén una versión formateada para presentación.")
...
def formatear_excel(df):
    ...
    sheet = libro["Sheet1"]  # Puedes cambiarlo por ws = wb['NombreHoja'] si conoces el nombre
    ...
    sheet.add_data_validation(lista)
    ...
    col_letter = sheet.cell(row=1, column=max_col).column_letter
    sheet.auto_filter.ref = f"A1:{col_letter}1"


    for fila in range(2, sheet.max_row + 1):
        sheet.row_dimensions[fila].height = 100
...
if archivo:
    ...
    libro_sd=load_workbook(archivo)
    ...
    st.write("Presiona el botón para generar el archivo formateado.")

    if st.button("Generar Excel Formateado"):
        excel_generado = formatear_excel(df_total)

        st.download_button(
            label="📥 Descargar Excel Formateado",
            data=excel_generado,
            file_name="excel_formateado.xlsx",
